...
class Simulation:
	...
	def __set_build_telem(self):
		panel_telems = {}
		for panel in self.model.panel_list:
	...
	def set_telemetry(self):
		...
		surf_telem['F'] = hull.F_surf
		surf_telem['M'] = hull.M_surf

		for wr in self.model.wing_roots:
			wr_telem = self.__telem['wing_roots'][int(not wr.left)]
		...
		self.__telem['running'] = self.is_running
		self.telem = json.dumps(self.__telem)
...
async def handler(socket: websockets.ServerConnection):
	sockets.add(socket)
	print(f'INFO: Connection from {socket.remote_address}')
